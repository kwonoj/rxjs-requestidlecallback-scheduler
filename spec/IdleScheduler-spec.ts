...
describe('Scheduler.Idle', () => {
  ...
  it('should execute recursively scheduled actions in separate asynchronous contexts', (done: MochaDone) => {
    let syncExec1 = true;
    let syncExec2 = true;
    idle.schedule(function (index: any) {
      if (index === 0) {
        this.schedule(1);
        idle.schedule(() => { syncExec1 = false; });
      } else if (index === 1) {
        this.schedule(2);
        idle.schedule(() => { syncExec2 = false; });
      } else if (index === 2) {
        this.schedule(3);
      } else if (index === 3) {
  ...
  it('should cancel the animation frame if all scheduled actions unsubscribe before it executes', (done: MochaDone) => {
    ...
    const action1 = idle.schedule(() => { idleExec1 = true; });
    const action2 = idle.schedule(() => { idleExec2 = true; });
